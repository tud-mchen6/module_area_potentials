...
def prepare_global_order(
    ...
    lcoe, prod, tech_id = _flatten_for_sort(ds, lcoe_var, prod_var, tech_dim, tech_names)
...
def plot_supply_curve_line(prep: Dict[str, np.ndarray],
    ...
    plt.xlabel(x_label)
    plt.ylabel("LCOE (same units as input, e.g., €/MWh)")
    plt.title(title)
...
def build_land_use_curve_points(
    ...
    # Productivity per area unit (guard zeros)
...
def plot_land_use_curve_line(
    ...
    x_label: str,
    output_path: str,
    area_label: str = "Cumulative land use (same units as 'pixel_area')",
    title: str = "Land-Use Curve (overlap → flat segments, then rise)",
